...
module streamlock::protocol {
    ...
    struct ViewingSession has store, drop, copy {
        session_id: u128,
        video_id: u128,
        viewer: address,
        creator: address,
    ...
    public entry fun register_video(
        ...
        assert!(vector::length(&key_commitment_root) == 32, errors::invalid_commitment());
    ...
    public entry fun start_session(
    ...
    ) acquires GlobalConfig, VideoRegistry, SessionRegistry {
        ...
        // Transfer funds to escrow (contract address)
        coin::transfer<AptosCoin>(viewer, @streamlock, prepaid_amount);
    ...
    public entry fun pay_for_segment(
        ...
        session_id: u128,
        segment_index: u64
    ) acquires VideoRegistry, SessionRegistry, Creator {
        let viewer_addr = signer::address_of(viewer);
        ...
        let already_paid = vector::contains(&session.paid_segments, &segment_index);
        assert!(!already_paid, errors::invalid_segment_index());
        ...
        vector::push_back(&mut session.paid_segments, segment_index);
        ...
        creator.total_earnings = creator.total_earnings + price;
        creator.pending_withdrawal = creator.pending_withdrawal + price;
    ...
    public entry fun top_up_session(
    ...
    ) acquires VideoRegistry, SessionRegistry {
        let viewer_addr = signer::address_of(viewer);
        ...
        // Transfer funds
        coin::transfer<AptosCoin>(viewer, @streamlock, additional_amount);
    ...
    public entry fun end_session(
        viewer: &signer,
        session_id: u128
    ) acquires GlobalConfig, SessionRegistry, VideoRegistry {
        let viewer_addr = signer::address_of(viewer);
        ...
        if (refund_amount > 0) {
            // In production, use a resource account to hold escrow funds
            // For now, we transfer from the contract address
            // This requires the contract to have funds
        };
    ...
    public entry fun withdraw_earnings(
        ...
        // In production, transfer from escrow resource account
        // coin::transfer<AptosCoin>(@streamlock, creator_addr, amount);
    ...
    public fun is_segment_paid(session_id: u128, segment_index: u64): bool acquires SessionRegistry {
        ...
        let session = table::borrow(&registry.sessions, session_id);
        vector::contains(&session.paid_segments, &segment_index)
    }
}
