...
import { InvalidProofError } from '@streamlock/common';
 ...
 * @param keys - Array of segment keys
 * @returns MerkleTree object with leaves, layers, and root
 */
 ...
 * @param index - Index of the key (0-based)
...
export function generateMerkleProof(tree: MerkleTree, index: number): MerkleProof {
  if (index < 0 || index >= tree.leaves.length) {
    throw new Error(`Invalid index: ${index}. Must be between 0 and ${tree.leaves.length - 1}`);
  }
 ...
 * @param key - The key to verify
 * @param proof - MerkleProof object
 * @throws InvalidProofError if proof is invalid
 */
...
/**
 * Serialize Merkle tree to JSON-safe object
 */
